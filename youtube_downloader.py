...
def download_video(url=None,playlist_url=None):
    ...
    if url:
        ...
        yt_video.streams.filter(progressive=True, file_extension='mp4').order_by('resolution')[-1].download(output_path=path)
        ...
        for video,number in zip(playlist,range(0,amount_of_videos)):
            ...
            yt.streams.filter(progressive=True, file_extension='mp4').order_by('resolution')[-1].download(filename=numbered_title,output_path=path)
